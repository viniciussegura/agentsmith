...
A **delivered item** is done at its own gate: a working spec reaching `Implemented` (#ai-plan), or a request landed without one -- a trivial change that skipped the spec (#ai-plan), or a line closed in `annotated.md` (#ai-multiple-requests).
The **branch** is done when every delivered item in it is done and the branch items below hold.
Do not open or update a PR before both tiers hold.

**Per delivered item.** Checked as each one lands.

1. The item is delivered as raised.
   A partial delivery is done only when the narrowing was surfaced and accepted (#ai-plan-deviation).
2. Tests for the item pass locally.
...
3. Documentation drift the item caused is resolved (#swe-docs-drift), including the reference spec when current behavior changed (#swe-reference-spec), the entity model when the schema changed (#swe-entity), and the design-decisions log when the item altered an existing decision's rationale (#swe-design-decisions).
...
5. The item has been self-reviewed against these instructions.

**Per branch.** Checked once, before the PR.

1. Branch consolidation is done (#swe-consolidation-audit), when the branch carries more than one delivered item and at least one of them required a working spec (#ai-plan).
