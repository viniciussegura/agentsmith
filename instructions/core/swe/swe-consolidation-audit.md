...
- **Consolidate live documents.**
    Live docs (_i.e._ documentation that reflects the current repository state, such as future work, technical debts, specs, and ADRs) **MUST** be left asserting the branch's final position.
...
Verify every finding before reporting it, and prefer a failing-first signal (a query count, a failing assertion) over "tests still pass".
