...
- The set of working specs is indexed at generated `docs/working-specs/INDEX.md`, regenerated with `agentsmith spec-index` on any structural change to the set -- a spec added, a spec directory deleted or renamed, a `Status:` changed, or a plan pruned. Regeneration is mechanical upkeep gated by #swe-done; `agentsmith spec-index --check` (run by a drift test) is the backstop, not the trigger.
