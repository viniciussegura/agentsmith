...
An unreleased shape may change freely; the versioning obligation attaches only once a shape has shipped to a consumer. A version increment is a release-time decision (#swe-done / #git-pr), never per-commit during development.
...
Define the two thresholds for this project in the reference spec (#swe-reference-spec): what marks a shape as **shipped to a consumer** and what one **release cycle** is. Absent a project definition, treat "shipped" as merged to the default branch and one release cycle as one tagged release.
