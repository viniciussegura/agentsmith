...
Each **round** is the engine's shared round, parameterized for the spec board: **Plan -> Review -> Reduce -> Persist**, with **no Verify** (spec findings are not adversarially re-verified; the generalist converge is the judgement). The per-round mechanics, the args descriptor, and the kickstart/routing contract are the unified ones in `#ai-review-engine` and `docs/reference-spec/review-board-protocol.md` (`specArgs` in `round-args.mjs` is the spec descriptor: `board:'spec'`, `maintainer:'spec-specialist'`, `verify:false`, `plan` enabled, `persistCmd` = `guard.mjs`). The main-thread driver below executes the **same** `plan -> review -> reduce -> persist` steps inline; the deterministic `/spec-review-board-wf` command runs exactly one such round via the `Workflow` driver. **The convergence loop is main-thread either way** -- the round driver never loops; the author revision + rebuttal + `guard.mjs` between rounds is the loop, and it lives here.
...
4. **Guard** (Persist). Run `node guard.mjs <scratch-dir> <n>` (add `--new-cycle` only when starting a fresh cycle). It merges the review into `ledger.json`, computes `b(n)`, updates `meta`, and prints the verdict. Read the printed verdict, not the ledger internals.
