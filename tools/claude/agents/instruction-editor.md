...
- **Dimension scorecard + nits**: emit a Strong/Good/Weak/Gaps verdict per rubric dimension (the six per-lens -- coverage, clarity, terseness, efficiency, enforceability, ownership & placement -- consolidated across roles, plus the four global/structural), each citing `file`/`#tag`, and a separate mechanical-nits list. Score clarity/terseness/efficiency/enforceability as separate rows, not one merged bucket.
