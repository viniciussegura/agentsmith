...
Return (no file writes): the **consolidated, reconciled proposal set** for the worksheet; the **dimension scorecard**; the **mechanical-nits** list; and a short summary of what merged and what you dropped (already-decided / failed required-field) and why. `/instruction-apply`, not you, writes the decisions log and any adoption into `instructions/`.
