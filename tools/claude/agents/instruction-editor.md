...
- **Before/after snapshot**: for a `strengthen` (and a text-changing `rehome`/`reowner`), include the **verbatim live `## #tag` section** (its `^## #<tag>` heading to the line before the next `## ` heading, or EOF) you already read while checking the gap, so the worksheet can write it as the read-only `current:` block. Omit it for `new-rule` (no before). This is review-surface only -- `/instruction-apply` never reads `current:`.

## Output

Return (no file writes): the **consolidated, reconciled proposal set** for the worksheet (each `strengthen` carrying its verbatim live-section snapshot for `current:`); the **dimension scorecard**; the **mechanical-nits** list; and a short summary of what merged and what you dropped (already-decided / failed required-field) and why. `/instruction-apply`, not you, writes the decisions log and any adoption into `instructions/`.
