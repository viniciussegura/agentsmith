...
0. **Resolve the install context.** Run `node <skills>/code-review-board/round-context.mjs`, where `<skills>` is wherever the board skills are installed. It prints `{agentPrefix, skillsDir, cwd}`; pass both to `specArgs` in step 3 rather than guessing. `agentPrefix` is `agentsmith:` on a plugin install and `''` on an npx/CLI install — a wrong value kills the round in Plan with `agent type not found` — and `skillsDir` makes every round command absolute, which Workflow subagents need since they do not inherit this cwd.
...
2. **Snapshot the pre-round git state** for the containment guard (reviewers carry the Write tool): run `node "<skillsDir>/code-review-board/round-guard.mjs" snapshot "<abs>/.agentsmith/tmp/spec-review/<spec-dir-name>/git-baseline.txt"`. `specArgs` defaults `guardBaseline` to this path; the driver's final **Guard** phase checks it (distinct from the spec convergence `guard.mjs` that the Persist step runs).
3. **Invoke the driver.** Build the round args with `specArgs({ roundId: '<n>', scratch: '<abs>/.agentsmith/tmp/spec-review/<spec-dir-name>', subjectRef: '<spec path>', candidateLenses: [<curated spec_review lenses>], agentPrefix, skillsDir })` from `round-args.mjs`. Invoke the `Workflow` tool with `scriptPath` = the installed `.claude/skills/code-review-board/board-round.mjs` and `args` = that object. `specArgs` carries `plan` (so the spec-specialist plans the consult set), `verify: false` (no Verify phase), `maintainer: 'spec-specialist'`, and `persistCmd` = `guard.mjs` (the Persist step runs the convergence guard).

4. **Read the verdict** (the Persist step already ran `guard.mjs <scratch> <n>` from `skillsDir`; read its printed verdict, not the ledger internals). First confirm the driver's **Guard** phase came back clean. Exit `1` means an agent wrote outside the gitignored scratch — stop and report the offending paths before acting on the verdict. Exit `3` means the guard could not find its baseline and **did not run**: a setup path mistake, not a containment violation, so fix the path and re-check rather than reporting an escape. Then:
