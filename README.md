...
The adapter install is namespaced and non-destructive: it writes only the adapter's own files (e.g. `.claude/skills/spec-review-board/`) and never touches the rest of a consumer's `.claude/`.
...
Commands then surface namespaced -- `/agentsmith:code-review-board`, `/agentsmith:spec-review-board`, `/agentsmith:agentsmith-init`. Updates: bump `version` in `package.json`, run `npm run build:plugin`, commit; users run `/plugin marketplace update`.
